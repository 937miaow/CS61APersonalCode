...
class BankAccount:
    ...
    def deposit(self, amount):
        ...
        temp = Transaction(len(self.transactions), before, self.balance)
    ...
    def withdraw(self, amount):
        ...
        if amount > self.balance:
            temp = Transaction(len(self.transactions), self.balance, self.balance)
            self.transactions.append(temp)
...
class Coin:
    ...
    def worth(self):
        "*** YOUR CODE HERE ***"
        if self.year + 50 >= Mint.present_year:
            return self.cents
